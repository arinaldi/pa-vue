...
export async function getArtistId(token: string, artist: string): Promise<string | null> {
...
export async function getArtistAlbums(token: string, artistId: string): Promise<Result[]> {
  const res = await fetch(
